...
def log_view(
    ...
    rgb_coarse = img_HWC2CHW(ret["outputs_coarse"]["rgb"].detach())
    if "depth" in ret["outputs_coarse"].keys():
        depth_pred = ret["outputs_coarse"]["depth"].detach().cpu().astype(np.uint8)
        depth_coarse = img_HWC2CHW(colorize(depth_pred, cmap_name="jet"))
    else:
        depth_coarse = None

    if ret["outputs_fine"] is not None:
        rgb_fine = img_HWC2CHW(ret["outputs_fine"]["rgb"].detach().cpu.astype(np.uint8))
        if "depth" in ret["outputs_fine"].keys():
            depth_pred = ret["outputs_fine"]["depth"].detach().cpu().astype(np.uint8)
            depth_fine = img_HWC2CHW(colorize(depth_pred, cmap_name="jet"))
    else:
        rgb_fine = None
        depth_fine = None

    rgb_coarse = (255/rgb_coarse.max().item()*rgb_coarse.permute(1, 2, 0).detach().cpu()).numpy().astype(np.uint8)
    ...
    if depth_coarse is not None:
        depth_coarse = depth_coarse.permute(1, 2, 0).detach().cpu().numpy().astype(np.uint8)
